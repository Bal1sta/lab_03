...
// noexcept говорит компилятору о том что ф-я не будет выбрасывать исключения
...
int main() { // Начало функции main
    ...
    arr.insert(arr.size() / 2, 20); // Вставляем число 20 в середину массива
    ...
    list.insert(arr.size() / 2, 4); // Вставляем число 4 в середину односвязного списка
    list.print(); // Печатаем содержимое односвязного списка
    ...
    doubleList.insert(arr.size() / 2, 4); // Вставляем число 4 в середину двусвязного списка 
